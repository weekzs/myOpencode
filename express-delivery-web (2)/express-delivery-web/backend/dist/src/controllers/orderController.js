...
class OrderController {
    ...
    async createOrder(req, res) {
        try {
            ...
            const order = await orderService_1.orderService.createOrder(req.user.id, orderData);
            res.status(201).json({
                success: true,
                order
            });
    ...
    async getUserOrders(req, res) {
        try {
            ...
            const orders = await orderService_1.orderService.getUserOrders(req.user.id, status);
            res.json({ orders });
        }
        catch (error) {
    ...
    async getOrderById(req, res) {
        try {
            const { id } = req.params;
            const order = await orderService_1.orderService.getOrderById(id, req.user.id);
            if (!order) {
    ...
    async updateOrderStatus(req, res) {
        try {
            ...
            const order = await orderService_1.orderService.updateOrderStatus(id, status, req.user.id);
            res.json({
                success: true,
                order
            });
    ...
    async cancelOrder(req, res) {
        try {
            const { id } = req.params;
            const order = await orderService_1.orderService.cancelOrder(id, req.user.id);
            res.json({
                success: true,
                order
            });
    ...
    async getOrderStats(req, res) {
        try {
            const stats = await orderService_1.orderService.getOrderStats(req.user.id);
            res.json({ stats });
        }
        catch (error) {
